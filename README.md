...
├── manifest.json           # PWA manifest (Telugu app name, icons, colors, display mode)
├── sw.js                   # service worker — caches everything for offline use
├── .nojekyll                # tells GitHub Pages not to run Jekyll on this folder
├── icons/
│   ├── icon-192.png         # app icon (joined-hands motif), used for install/home-screen
│   ├── icon-512.png
│   └── apple-touch-icon.png
└── images/
    ├── ganapathi.jpg         # top-left letterhead avatar
    ├── dattatreya.jpg        # top-right letterhead avatar
    └── hands-badge.png       # small badge under the letterhead, above the title
```

Everything is static — there is no backend, database, or build step. Any static host works.

**Important when re-uploading to GitHub:** both `icons/` and `images/` must stay as actual folders.
If you use "Add file → Upload files" and drag individual files instead of folders, type the folder
prefix (e.g. `images/ganapathi.jpg`) into each file's name box before committing — otherwise GitHub
will drop the files in the repo root instead of inside the folder, and the app will show broken
images (this is exactly what happened with the icons folder the first time).
